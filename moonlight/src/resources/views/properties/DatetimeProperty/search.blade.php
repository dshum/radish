...
<div{!! $from || $to ? '' : ' class="dnone"' !!} container="property" property="{{ $name }}">
    <input type="text" name="{{ $name }}-from" value="{{ $from ? $from->format('Y-m-d') : '' }}" class="date" placeholder="ГГГГ-ММ-ДД" readonly{!! $from ? '' : ' disabled="disabled"' !!}> &#151;
    <input type="text" name="{{ $name }}-to" value="{{ $to ? $to->format('Y-m-d') : '' }}" class="date" placeholder="ГГГГ-ММ-ДД" readonly{!! $to ? '' : ' disabled="disabled"' !!}>
</div>