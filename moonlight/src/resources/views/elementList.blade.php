@if (sizeof($elements))
    @if ($currentPage == 1)
    <div class="count">Всего {{ $total }} {{ Moonlight\Utils\RussianTextUtils::selectCaseForNumber($total, ['элемент', 'элемента', 'элементов']) }}.</div>
    @else
    ...
    <li elementid="Agency.1">
        ...
        <div><a href="{{ route('element', $element->getClassId()) }}">{{ $element->{$currentItem->getMainProperty()} }}</a></div>
    </li>
    @endforeach
    </ul>
    @if ($hasMorePages)
